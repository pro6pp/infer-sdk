...
/**
 * Extracts the address from the API place subtitle.
 *
 * The API subtitle is formatted as `category · address`; the SDK only renders
 * the address because the place name already identifies the result.
 */
export function getPlaceAddress(subtitle?: string | null): string {
  if (!subtitle) return '';

  const separatorIndex = subtitle.indexOf('·');
  return (separatorIndex === -1 ? subtitle : subtitle.slice(separatorIndex + 1)).trim();
}
