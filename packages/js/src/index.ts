...
const activeInstances = new WeakMap<HTMLElement, InferJS>();

function getActiveInstance(target: HTMLElement): InferJS | undefined {
  return activeInstances.get(target);
}

function registerInstance(instance: InferJS, ...elements: HTMLElement[]): void {
  elements.forEach((element) => activeInstances.set(element, instance));
}
...
export class InferJS {
  ...
  constructor(target: string | HTMLElement, config: InferJSConfig) {
    ...
    const existingInstance = getActiveInstance(el as HTMLElement);
    if (existingInstance) {
      existingInstance.destroy();
    }

    this.targetElement = el as HTMLElement;
    this.ownsInput = !(el instanceof HTMLInputElement);
    ...
    this.core = new InferCore({
      ...
      onPlaceSelect: (selection) => {
        if (selection && this.core.state.query) {
          this.input.value = this.core.state.query;
        }
        if (config.onPlaceSelect) config.onPlaceSelect(selection);
      },
    });
    ...
    this.bindEvents();
    registerInstance(this, this.targetElement, this.input);
  }
  ...
  /**
   * Retrieves the current selected place value if available.
   */
  public get place(): PlaceValue | null {
    return this.core.state.selectedPlace || null;
  }
  ...
  /**
   * Programmatically selects a place value.
   */
  public setPlace(place: PlaceValue): void {
    if (!place) return;
    ...
    this.core.selectItem(suggestion);
  }
  ...
  public destroy() {
    ...
    activeInstances.delete(this.targetElement);
    activeInstances.delete(this.input);
  ...
  private render(state: InferState) {
    ...
    const items = getOrderedItems(state);
    ...
    items.forEach((item, index) => {
      ...
      if (isPlace) {
        li.classList.add('pro6pp-item--place');
      }
      ...
      if (isFuzzy) {
        li.classList.add('pro6pp-item--fuzzy');
      }
      ...
      const segments = isFuzzy
        ? getFuzzyHighlightSegments(item.label, state.query)
        : getHighlightSegments(item.label, state.query);
      ...
      const secondaryText = !isPlace ? item.subtitle || countVal : '';

      if (isPlace && placeAddress) {
        const subSpan = document.createElement('span');
        subSpan.className = 'pro6pp-item__subtitle';
        subSpan.textContent = `, ${placeAddress}`;
